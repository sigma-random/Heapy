...
def coalesc(state,consolidate):
    ...
    for i,chunk in enumerate(state):
        if chunk.status == "allocated" or ( chunk.type == "fast_chunks" and consolidate == False ): # stop collecting address to coalesc in this cases
            ...
            if len(tocoalesc) > 1: # we have some chunks to coalesc!
                ...
                for c_index in tocoalesc:
                    ...
                    new_raw_size += chunk_to_merge.raw_size
